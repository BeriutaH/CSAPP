...
int tmult_ok(int x, int y) {
    ...
    printf("p (64-bit) in decimal: %" PRId64 "\n", p);  // 打印 64 位值
    printf("(int)p (32-bit): %d\n", (int)p);  // 打印 32 位值
    return p != (int) p;
}
...
// 2.33
/* −x 是 x 的 加法逆元，最小值的加法逆元是它本身，对于其他 x 和 -x 互为加法逆元
 * 这种对称性使得补码加法系统形成一个阿贝尔群
     * 用一个十六进制数字来表示长度w=4的位模式，根据这些数字的补码的解释，填写下表，确定所示数字的加法逆元
     * w = 4, 最大值 (2^4-1) -1 = 7, 最小值-2^4-1 = -8
     *              x                              -t/4x
     *     十六进制              十进制              十进制        十六进制
     *   0[0000]                 0                   0             0
     *   5[0101]                 5                  -5取反+1        B
...
// 2.34
/* 按照2-27的风格填写下表，说明不同的3位数字乘法的结果，
     * 反码是取反的时候去除符号位，其余的取反，+1
     * 补码的取反是全部按位取反，包括符号位，+1
 *      模式          x                       y               x*y (负数*负数得正数)          截断的x*y
 -----------------------------------------------------------------------------------------------------
 *     无符号        [100]        4        [101]  5             20[010100]                  [100]4
 *     补码         [100]-2^2    -4        [101] -3            12[001100]                  [100]-4
 -----------------------------------------------------------------------------------------------------
 *     无符号        [010]        2        [111] 7              14[001110]                  [110]6
 *     补码         [010]        2        [111] -1             -2[111110]                  [110]-2
 -----------------------------------------------------------------------------------------------------
 *     无符号        [110]        6        [110] 6             36[100100]                  [100]4
 *     补码         [110]取反+1  -2        [110] -2             4[000100]                  [100]-4
 -----------------------------------------------------------------------------------------------------
*/

// 2.36
/* 32位转化成64的乘积，如果转化成32位，跟原本的数字一样大，就是没有溢出，反之溢出
    printf("%d\n", tmult_ok(1000000, 20000));  // 溢出
    printf("%d\n", tmult_ok(INT32_MAX, 1)); // 未溢出
    printf("%d\n", tmult_ok(INT32_MAX, 2)); // 溢出
*/

// 2.37
/* 现在你有一个任务，当数据类型 int 和 size_t 都是32位的，修补上述旁注给出的XDR代码中的漏洞。
 * 你决定将待分配字节数设置为数据类型 uint64_t， 来消除乘法溢出的可能性。
 *  A: 这段代码对原始代码有了哪些改进？
        * 这个改动完全没有帮助，虽然 asize 的计算会更准确，
        * 但是调用 ma1loc 会导致这个值被转换成一个 32位无符号数字，因而还是会出现同样的溢出条件。
 * B: 该如何修改代码来消除漏洞
        * 直接判断是否溢出，如果溢出，直接返回
        uint64_t required_size = ele_cnt * (uint64_t)ele_size;  // 计算所需内存大小

        // 转换到 size_t 类型,如果 required_size 的值超过 size_t 的最大表示范围 那么会发生截断
        ...
        // 在确认内存大小没有溢出的情况下，调用 malloc 分配内存,如果 malloc 返回 NULL，说明内存分配失败（例如内存不足）
...
// 2.40
/* 基本原理：
    * 整数乘法可以分解为移位操作和加法/减法的组合
    * 因为 x * 2^n 可用 x << n来表示，所以我们可以用移位代替乘以 2 的幂
    * 对非 2 的幂的常数 K ,需要将其分解为多项和/差，每一项都可以用移位表示
 * 表格字段解释
    * K: 目标常数
    * 移: 需要的移位操作次数
    * 加法/减法: 需要的加法或减法操作次数
    * 表达式: 用移位和加减法实现 x * K 的具体公式
 * 填写下表
    *      K          移位        加法/减法                表达式
    *      6           2            1         (x<<2)+(x<<1)  === 2^2 + 2^1 = 6
    *      31          1            1         (x<<5)- x  ===  2^5 - 2^0 = 32-1
    *      -6          2            1         正确: (x<<1)-(x<<3) ===  2^1 - 2^3  = -6   (做错) -(x<<2)+(x<<1)  ===  -(2^2 + 2^1) = -6
    *      55          2            2         (x<<6)-(x<<3)- x  ===  2^6 - 2^3 - 2^0 = 64-8-1
*/

// 2.41
/*  对于一组从位位置n开始到位位置m 的连续的1（n≥m），我们看到可以产生两种形式的代码，
 *  A和B。编译器该如何决定使用哪一种呢？
 *  怎么查找n和m。例如：44
     *  将 K 转换为二进制: 101100
     *  找出最高位的1, 2^n+1 一个大于等于 44 的 最小 2 的幂次，因为最高位是5 n+1= 5+1 = 6
     *  找出需要减去的次高位（对应2^m）, 次高m=4
 *
*/
}
